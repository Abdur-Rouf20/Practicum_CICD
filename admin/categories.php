...
require_once "auth_check.php"; // ✅ This handles admin login check
        ...
        <div class="col-md-4">
            <select name="parent" class="form-control">
                ...
                <?php foreach ($parents as $p): ?>
                    <option value="<?= $p['id'] ?>" <?= (($_GET['parent'] ?? '') == $p['id']) ? 'selected' : '' ?>>
                        <?= $p['name'] ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <ul class="pagination justify-content-center">
            <?php for ($i = 1; $i <= $pages; $i++): ?>
                <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                    <a class="page-link" 
                       href="?page=<?= $i ?>&search=<?= $_GET['search'] ?? '' ?>&parent=<?= $_GET['parent'] ?? '' ?>">
                        <?= $i ?>
                    </a>
                </li>
            <?php endfor; ?>
